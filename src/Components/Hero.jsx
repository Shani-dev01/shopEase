...
>
  {product.map((items) => (
    <SwiperSlide key={items.id} className="h-full">
      ...
      <div className="h-full w-full bg-gradient-to-r from-slate-950 via-slate-800 to-slate-600 flex items-center justify-between px-10 md:px-20">

        {/* LEFT */}
        <div className="h-full w-1/2 flex flex-col justify-center text-white">
          <span className="mb-4 w-fit px-4 py-2 rounded-full bg-white/10 text-sm backdrop-blur">
          ...
          <div className="flex items-center gap-5">
            ...
            <button className="px-6 py-3 rounded-full bg-white text-black font-semibold hover:bg-gray-500 transition cursor-pointer ">
              Shop Now →
            </button>
          </div>
        </div>
